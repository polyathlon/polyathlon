...
{
    [ViewType("CategoryCollectionView")]
    public partial class CategoryCollectionView : BaseViewWithWinUIButtons, ISupportParameter
    {
        ...
        {
            var fluentAPI = mvvmContext.OfType<CategoryCollectionViewModel>();
            mvvmContext.BindCollectionGrid<CategoryCollectionViewModel, CategoryViewEntity, Category>(gridView, categoryBindingSource);

            fluentAPI.SetObjectDataSourceBinding(categoryBindingSource, x => x.Entities);
            fluentAPI.SetBinding(categoryBindingSource, rbs => rbs.DataSource, x => x.Entities);
